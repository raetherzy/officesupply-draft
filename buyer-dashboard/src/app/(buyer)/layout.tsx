...
import { BuyerRole } from "@/lib/types";
...
export default function BuyerLayout({ children }: { children: ReactNode }) {
  ...
  const [activeRole, setActiveRole] = useState<BuyerRole>(currentUser.role);
  const [sidebarOpen, setSidebarOpen] = useState(true);

  const visibleMenu = menuItems.filter((item) => item.roles.includes(activeRole));
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="border-b border-[#e5e5e5] px-5 py-3">
            ...
            <p className="mt-1 text-sm font-medium text-black">PT Nirmana Sthavira Dharma</p>
          </div>

          {/* Nav */}
          <nav className="flex-1 overflow-y-auto px-3 py-3">
            {visibleMenu.map((item) => {
              const exactMenuItemMatch = menuItems.some((m) => pathname === m.href);
              const isActive = exactMenuItemMatch
                ? pathname === item.href
          ...
          <div className="border-t border-[#e5e5e5] px-5 py-4">
            ...
            <select
              value={activeRole}
              onChange={(e) => setActiveRole(e.target.value as BuyerRole)}
            ...
            >
              ...
              <option value="finance_buyer">Finance Buyer</option>
            </select>
          </div>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-30 border-b border-[#e5e5e5] bg-white/80 backdrop-blur-sm">
          <div className="flex items-center justify-between px-6 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-medium uppercase tracking-widest text-[#888888]">Buyer Console</p>
                <p className="text-sm font-medium text-black">{currentUser.company_name}</p>
              </div>
            </div>
